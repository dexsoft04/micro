...
if [[ $MICRO_ENV == "dev" ]]; then
  ...
  overrides="--set persistence.size=10Gi --set s3gateway.serviceEndpoint=$S3_ENDPOINT --set s3gateway.accessKey=$S3_ACCESS_KEY --set s3gateway.secretKey=$S3_SECRET_KEY"
fi

helm install minio-cluster bitnami/minio --version 14.3.0 $overrides -f values.yaml \
